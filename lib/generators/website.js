...
Generator.prototype.convertFile = function(input) {
    ...
    .then(function(page) {
        ...
        return that._writeTemplate(that.templates["page"], {
            ...
            basePath: basePath,
            staticBase: links.join(basePath, "zhibimo")
        }, output);
    });
};
...
Generator.prototype.copyAssets = function() {
    ...
    return fs.copy(
        ...
        path.join(that.options.output, "zhibimo")
    )
    ...
    .then(function() {
        return Q.all(
            _.map(that.book.plugins.list, function(plugin) {
                var pluginAssets = path.join(that.options.output, "zhibimo/plugins/", plugin.name);
